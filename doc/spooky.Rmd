...
This data originally comes from a Kaggle dataset we'll call the Spooky Dataset. This was part of a [competition](https://www.kaggle.com/c/spooky-author-identification) that happened in late 2017.

## Basic issues

* Are we doing analysis at the word, phrase, sentence, or corpus level?

* I'm noticing there are more interesting and sensitive analyses I might like to do, but I'm running up against some limits in terms of trying to become more sophisticated at writing in `R` and in terms of time! 

** I recognize that a lot of this code follows the pattern of naming variables, executing functions, and drawing up plots three times, but I'm not good enough at R to do the string formatting necessary to condense all of this into individual function calls that operate on all three authors.
...
```{r Create wordcloud PDFs, message=FALSE}
...
```{r Write piechart PDF, message=FALSE}
pdf("../figs/g_total_gen_pron.pdf")
g_total_gen_pron
dev.off()
```
...
ggsave("../figs/g_EAP_gen_pron.pdf", g_EAP_gen_pron, device = "pdf")
ggsave("../figs/g_HPL_gen_pron.pdf", g_HPL_gen_pron, device = "pdf")
ggsave("../figs/g_MWS_gen_pron.pdf", g_MWS_gen_pron, device = "pdf")
```
...
We'll create topic models for each author using the Correlated Topic Models algorithm, and see how they might be different or similar to those generated from LDA>
```{r}
