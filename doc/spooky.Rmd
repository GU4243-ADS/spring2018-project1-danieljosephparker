...
```{r Wordclouds of most common words, warning=FALSE}
...
```{r Create wordcloud PDFs & PNGs, message=FALSE, warning=FALSE}
...
```{r Wordclouds of less common words, warning=FALSE}
...
```{r Create more wordcloud PDFs, message=FALSE, warning=FALSE}
...
```{r Wordclouds of really uncommon words, warning=FALSE}
...
```{r Generate more wordcloud PDFs, message=FALSE, warning=FALSE}
...
```{r Save bigram wordclouds, message=FALSE, warning=FALSE}
...
```{r Formality calculation, message=FALSE}
...
```{r Generate formality plots, eval=FALSE, include=TRUE}
g_EAP_form <- plot(EAP_formality)
g_HPL_form <- plot(HPL_formality)
g_MWS_form <- plot(MWS_formality)
```

```{r Visualize formalities, eval=FALSE, include=TRUE}
...
```{r Run LDA with Gibbs sampling, eval=FALSE}
...
```{r Save LDA object externally, eval=FALSE, include=TRUE}
...
```{r Save CTM object externally, eval=FALSE, include=TRUE}
# In order to knit this markdown document into HTML more quickly,
# we'll just save the generated topic model object so that we
# don't have to run the algorithm every time.
